...
stdenv.mkDerivation {
  ...
  passthru = {
    ...
    inherit bintools;
    inherit libc nativeTools nativeLibc nativePrefix isGNU isClang isIntel isOneApi;
  ...
  installPhase =
    ''
      ...
      wrap() {
        ...
        local wrapper="$2"
        export prog="$3"
        export use_response_file_by_default=${if (isClang && (!(isIntel || isOneApi))) then "1" else "0"}
    ...
    + ''
      ...
      elif [ -e $ccPath/icx ]; then
        wrap ${targetPrefix}icx $wrapper $ccPath/icx
        ln -s ${targetPrefix}icx $out/bin/${targetPrefix}cc
        export named_cc=${targetPrefix}icx
      ...
      if [ -e $ccPath//intel64/icc ]; then
        wrap ${targetPrefix}icc $wrapper $ccPath/intel64/icc
        export named_cc=${targetPrefix}icc
      fi
      ...
      elif [ -e $ccPath/icpx ]; then
        wrap ${targetPrefix}icpx $wrapper $ccPath/icpx
        ln -s ${targetPrefix}icpx $out/bin/${targetPrefix}c++
        export named_cxx=${targetPrefix}icpx
      elif [ -e $ccPath/clang++ ]; then
        wrap ${targetPrefix}clang++ $wrapper $ccPath/clang++
    ...
    + optionalString cc.langFortran or false ''
      ...
      if [ -e $ccPath/${targetPrefix}ifx ]; then

...
, isGNU ? false, isClang ? cc.isClang or false, gnugrep ? null, gnused ? null
, buildPackages ? {}
, libcxx ? null
}:
...
stdenv.mkDerivation {
  ...
  installPhase =
    ...
    + ''
      ...
      elif [ -e $ccPath/pgcc ]; then
        wrap ${targetPrefix}pgcc ${./cc-wrapper.sh} $ccPath/pgcc
        ln -s ${targetPrefix}pgcc $out/bin/${targetPrefix}cc
        export named_cc=${targetPrefix}pgcc
        export named_cxx=${targetPrefix}pgc++
      fi
      ...
      elif [ -e $ccPath/pgc++ ]; then
        wrap ${targetPrefix}pgc++ $wrapper $ccPath/pgc++
        ln -s ${targetPrefix}pgc++ $out/bin/${targetPrefix}c++
      fi
    ...
    + optionalString cc.langFortran or false ''
      ...
      if [ -e $ccPath/${targetPrefix}flang ]; then
        wrap flang $wrapper $ccPath/flang
        export named_fc=${targetPrefix}flang
      fi
      if [ -e $ccPath/${targetPrefix}pgf90 ]; then
        wrap pgf90 $wrapper $ccPath/pgf90
        wrap pgf77 $wrapper $ccPath/pgf77
        export named_fc=${targetPrefix}pgf90
      fi
    ''
  ...
  postFixup =
    ...
    '' + optionalString (!(cc.langD or false)) ''
      echo "-I ${libc_dev}${libc.incdir or "/include"}" >> $out/nix-support/libc-cflags
    '' + optionalString (isGNU && (!(cc.langD or false))) ''
      for dir in "${cc}"/lib/gcc/*/*/include-fixed; do
        echo '-I' ''${dir} >> $out/nix-support/libc-cflags
      done
    '' + ''

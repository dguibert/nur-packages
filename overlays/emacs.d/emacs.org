...
- [[#denote-note-taking][Denote (note taking)]]
  - [[#denote-silo-formally-denote-silo-extras][Denote-silo (formally denote-silo-extras)]]
...
- [[#bibtex][Bibtex]]
- [[#lsp-language-server-server][LSP (Language-Server Server)]]
- [[#godbolt-compiler-explorer-for-emacs][Godbolt compiler-explorer for Emacs]]
  - [[#configuration][Configuration]]
  - [[#integrations-cc][Integrations: C/C++]]
...
Adapted from https://help-gnu-emacs.gnu.narkive.com/OFEfTzJt/ediff-from-command-line
#+begin_src emacs-lisp
(defun command-line-sidebyside (switch)
  (let ((file1 (find-file (pop command-line-args-left)))
        (file2 (find-file (pop command-line-args-left))))
    (delete-other-windows)
    (set-window-buffer (selected-window) file1)
    (set-window-buffer (split-window-horizontally) file2)
    ))
#+end_src

#+begin_src emacs-lisp
(add-to-list 'command-switch-alist '("O" . command-line-sidebyside))
#+end_src
...
#+begin_src emacs-lisp
;; Org Mode Configuration ------------------------------------------------------
(defvar dg/denote-base-dir
  (file-truename
      "~/Documents/denotes/")
  "Where I put my notes; I need to provision differently for personal and work computers.")
...
(use-package org
  ...
  ;(setq org-agenda-files '("~/Documents/denotes"))
  (defun dg/org-mode/agenda-files ()
    ...
    (let ((projects
           (mapcar (lambda (el) (cdr el)) (dg/project/list-projects))))
  ...
  (cl-defun dg/project/list-projects (&key (project ".+")
                                       (directory dg/denote-base-dir))
...
otherwise."
    (mapcar (lambda (line)
      ...
      (split-string-and-unquote
        (shell-command-to-string
          (concat
            "fd _project " directory " | xargs rg \"^#\\+PROJECT_NAME: +(" project ") *$\" "
            " --ignore-case --follow --only-matching --no-ignore-vcs --with-filename "
            "-r '$1' | tr '\n' '#'"))
        "#")))

  (setq org-agenda-files (dg/org-mode/agenda-files))
  ...
  (setq org-capture-templates
        '(("t" "todo"         entry (file "~/Documents/denotes/20241004T211931--inbox__project.org")
           "* TODO %?\n%U\n- ref :: %a\n")
          ;; time tracker (clocked tasks)
          ("g" "geek"         entry (file+olp+datetree "~/Documents/denotes/20240522T224517--tracker__project.org")
           "* GEEK %?         :perso:\n%U\n- ref :: %a\n"
           :prepend t :tree-type week :clock-in t :clock-keep t)
          ("c" "chat"         entry (file+olp+datetree "~/Documents/denotes/20240522T224517--tracker__project.org")
           "* CHAT %?         :work:chat:\n%U\n- ref :: %a\n"
           :prepend t :tree-type week :clock-in t :clock-keep t)
          ("e" "email"        entry (file+olp+datetree "~/Documents/denotes/20240522T224517--tracker__project.org")
           "* EMAIL %?        :work:email:\n%U\n- ref :: %a\n"
           :prepend t :tree-type week :clock-in t :clock-keep t)
          ("m" "meeting"      entry (file+olp+datetree "~/Documents/denotes/20240522T224517--tracker__project.org")
           "* MEETING %?      :work:meeting:\n%U\n- ref :: %a\n"
           :prepend t :tree-type week :clock-in t :clock-keep t)
          ("r" "review"       entry (file+olp+datetree "~/Documents/denotes/20240522T224517--tracker__project.org")
           "* REVIEW %?       :work:review:\n%U\n- ref :: %a\n"
           :prepend t :tree-type week :clock-in t :clock-keep t)
          ("w" "work"         entry (file+olp+datetree "~/Documents/denotes/20240522T224517--tracker__project.org")
           "* IN-PROGRESS %?  :work:\n%U\n- ref :: %a\n"
           :prepend t :tree-type week :clock-in t :clock-keep t)
          ("p" "pause"        entry (file+olp+datetree "~/Documents/denotes/20240522T224517--tracker__project.org")
           "* PAUSE %?        :pause:\n%U\n- ref :: %a\n"
           :prepend t :tree-type week :clock-in t :clock-keep t)
          ("i" "interruption" entry (file+olp+datetree "~/Documents/denotes/20240522T224517--tracker__project.org")
          ...
          ("S" "sport (planned)" entry (file+olp+datetree "~/Documents/roam/sport.org")
           ...
           :prepend t :tree-type month :jump-to-captured t)
          ("f" "chore"        entry (file "~/Documents/denotes/20241004T211931--inbox__project.org")
           "* IN-PROGRESS %?  :chore:\n%U\n"
...
** Denote-silo (formally denote-silo-extras)
...
(use-package denote-silo
  ...
  (dg/leader-keys
    "n s" #'denote-silo-open-or-create
    )
  )
#+end_src
...
(use-package cook-mode :ensure t
...
* Godbolt compiler-explorer for Emacs

[[https://gitlab.com/jgkamat/rmsbolt][RMSBolt]] is a supercharged implementation of the [[https://github.com/mattgodbolt/compiler-explorer][godbolt compiler-explorer]] for Emacs.
...
C/C++ will look for a compile_commands.json file in any directory higher than the source file. Once it finds one, it will parse the file trying to find a match for the current file, and if it finds one, it will compile using the flags present there. Many files (like headers) are not present in compile_commands.json and will not work (at this time).

Because the compilation commands in compile_commands.json assume the files are compiled in order, please make sure to fully compile the project before beginning, otherwise, you could see compiler errors.

In order to transform the given file, some flags are removed. This is the most brittle part of the current system, and could cause compile errors.

On projects with a lot of imports, the resulting assembly file might be large, which may cause Emacs to hang.
*** QuickStart Guide

- Generate a compile_commands.json file, and ensure it is in a domineering directory.

  #+begin_src sh
cmake -DCMAKE_EXPORT_COMPILE_COMMANDS=1 .
  #+end_src

- Compile the project completely. If you pass flags to your build system, make sure they are passed when generating compile_commands.json as well.
- Open the file in question, and enable `rmsbolt-mode`.
- Run `rmsbolt-compile`.

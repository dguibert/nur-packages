...
with (import ./slurm.nix { inherit pkgs date; });
with (import ./bench-helpers.nix { inherit pkgs; });
with pkgs; let

  #bench-helpers = callPackage ./bench-helpers { inherit pkgs; };
  jobs = recurseIntoAttrs (rec {
   #node_check = lib.genAttrs [ "SKL-20c_edr-ib2_192gb_2666" ] (partition: lib.genAttrs partitions."${partition}".NodeSet (node:
   node_check = lib.genAttrs (lib.mapAttrsToList (n: v: n) partitions)
     (partition: lib.genAttrs partitions."${partition}".NodeSet
       (node:
         (runJob { name="node-check-${node}-${date}";
    ...
    default_sbatch_genji = {
      job-name="bash";
      nodes="1";
      partition=partitions.SKL-20c_edr-ib2_192gb_2666.name;
      time="00:03:00";
      exclusive=true;
      verbose=true;
      no-requeue=true;

...
with final; with jobs.scheduler_slurm;

{
      ...
      scontrol_show_py = writeScript "scontrol-show.py" ''
        ...
        try:
          ...
          if len(all_) > 0:
            ...
            #display(all_)
      '';
    in runCommand "${json_file}" { buildInputs = with final; [ coreutils jq pythonPackages.pyslurm ];
  ...
  partitions = let
    ...
    in lib.mapAttrs extendNodeSet (builtins.fromJSON (builtins.readFile final.jobs.scheduler_slurm.partitions_json));
